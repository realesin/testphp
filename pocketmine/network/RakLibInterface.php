...
class RakLibInterface implements ServerInstance, AdvancedSourceInterface{
	...
	public function putPacket(Player $player, /*DataPacket*/ $packet, $needACK = false, $immediate = false){
	    if($player->getProtocol() == 84) {
            ...
            return null;
        } else if(AnyVersionManager::isProtocol($player, "0.14")){
            if(isset($this->identifiers[$h = \spl_object_hash($player)])){
                if($packet) {
                    $identifier = $this->identifiers[$h];
                    $pk = \null;
                    ...
                    if (!$immediate and !$needACK and $packet::NETWORK_ID !== \pocketmine\network\protocol\p70\Info::BATCH_PACKET
                        and Network::$BATCH_THRESHOLD >= 0
                        and \strlen($packet->buffer) >= Network::$BATCH_THRESHOLD) {
	...
	private function getPacket($buffer){
		...
		if(($data = $this->network->getPacket($pid)) and $data instanceof DataPacket){
