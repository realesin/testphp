...
class SessionManager
{
	...
	private function tick()
	{
		...
		if (($this->ticks & 0b1111) === 0) {
			$diff = max(0.001, $time - $this->lastMeasure);
	...
	public function blockAddress($address, $timeout = 3600){
		...
		if(!isset($this->block[$address]) or $timeout === -1){
			if($timeout === -1){
				...
				$this->getLogger()->notice("§6Anti-DDoS IP atacando: §e $address §6 por §e $timeout §6segundos");
				$d = date("m.d.y H:i:s");
                $ab = @fopen("RakLib.log","a+");
                fwrite($ab,"\n[$d] Anti-DDoS IP atacando: $address bloqueado por §e $timeout segundos");
                fclose($ab);
			}
